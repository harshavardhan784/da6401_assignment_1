...
# Optimizers available: sgd | momentum | nag | rmsprop
...
### Q2.3 — Optimizer Showdown (same architecture, all 4 optimizers)
...
python -m train -d mnist -e 10 -b 32 -o rmsprop -lr 0.1 \
    -nhl 3 -sz 128 128 128 -a relu -w_i xavier \
...
Logs: wandb confusion matrix, per-class accuracy bar chart,
      misclassified-examples grid (true class × top-5 confident failures).
...
Uses --log_neuron_grads → logs per-neuron grad norms for first 5 neurons
in every hidden layer at every iteration for the first 50 steps.
...
Based on MNIST sweeps, pick 3 configurations:
  1. RMSProp + ReLU + 3L×128        (best overall MNIST performer)
  2. NAG     + ReLU + 4L×128 + wd   (regularized deeper net)
